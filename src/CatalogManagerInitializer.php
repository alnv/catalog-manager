...
class CatalogManagerInitializer
{
    ...
    protected IconGetter $objIconGetter;

    protected array $arrCatalogs = [];

    protected array $arrCoreTables = [];

    protected array $arrBackendModules = [];

    protected I18nCatalogTranslator $objI18nCatalogTranslator;

    protected array $arrActiveBackendModules = [];

    public function initialize(): void
    {

        $blnInitializeInBackend = System::getContainer()->get('contao.routing.scope_matcher')->isBackendRequest(System::getContainer()->get('request_stack')->getCurrentRequest() ?? Request::create(''));
        ...
        if (Config::get('_isBlocked')) {
            ...
            System::getContainer()
                ...
                ->log(LogLevel::ERROR, $strMessage, ['contao' => new ContaoContext(__CLASS__ . '::' . __FUNCTION__)]);
        }
    }
        ...
        if (!empty($arrNavigationAreas) && is_array($arrNavigationAreas)) {
            ...
            foreach ($arrNavigationAreas as $intIndex => $arrNavigationArea) {
                ...
                if (!Toolkit::isEmpty($arrNavigationArea['key'])) {
                    ...
                    ArrayUtil::arrayInsert($GLOBALS['BE_MOD'], $intIndex, $arrNav);
    ...
    protected function setBackendModules(): void
    {
        ...
        foreach ($this->arrCatalogs as $strTablename => $arrCatalog) {
            ...
            if ($arrCatalog['isBackendModule'] && Toolkit::isEmpty($arrCatalog['pTable'])) {
                ...
                ArrayUtil::arrayInsert($GLOBALS['BE_MOD'][$strArea], $intIndex, $arrBackendModule);
    ...
    protected function createBackendModuleDc($strTablename, $arrCatalog): array
    {
        ...
        $arrModule['stylesheet'] = 'bundles/alnvcatalogmanager/catalog.css';
    ...
    protected function existContentElementInChildrenCatalogs($arrTables): bool
    {
    ...
    protected function getNestedChildTables(&$arrTables, $arrChildTables, $strTable = ''): void
    {
